...
class Question(commands.Cog):
    ...
    async def dice(self, ctx):
        '''Rolls the dice'''
        embed = Embed(title=f":game_die: You rolled {random.randint(1, 6)}", color=0x00ff10)
        await ctx.send(embed=embed)

    @commands.command(aliases=['flip'])
    async def coin(self, ctx):
        '''Flips a coin'''
        embed = Embed(title=f"You flipped {random.choice(['Heads', 'Tails'])}", color=0x3781ff)
        await ctx.send(embed=embed)

    @commands.command(aliases=['owner', 'creator'])
    async def boss(self, ctx):  # passing context
        '''Displays owner name'''
        embed = Embed(title=f'Nyx made me! He is my boss', color=0x457832)
        await ctx.send(embed=embed)
